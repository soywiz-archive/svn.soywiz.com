...
module pspsdk.pspiofilemgr_kernel;
...
struct PspIoDrvFuncs
{
	...
	int (*IoOpen)(PspIoDrvFileArg *arg, byte *file, int flags, SceMode mode); 
	int (*IoClose)(PspIoDrvFileArg *arg); 
	int (*IoRead)(PspIoDrvFileArg *arg, byte *data, int len); 
	int (*IoWrite)(PspIoDrvFileArg *arg, byte *data, int len); 
	SceOff (*IoLseek)(PspIoDrvFileArg *arg, SceOff ofs, int whence); 
	int (*IoIoctl)(PspIoDrvFileArg *arg, uint cmd, void *indata, int inlen, void *outdata, int outlen);
	int (*IoRemove)(PspIoDrvFileArg *arg, byte *name); 
	int (*IoMkdir)(PspIoDrvFileArg *arg, byte *name, SceMode mode); 
	int (*IoRmdir)(PspIoDrvFileArg *arg, byte *name);
	int (*IoDopen)(PspIoDrvFileArg *arg, byte *dirname); 
	int (*IoDclose)(PspIoDrvFileArg *arg);
	int (*IoDread)(PspIoDrvFileArg *arg, SceIoDirent *dir);
	int (*IoGetstat)(PspIoDrvFileArg *arg, byte *file, SceIoStat *stat);
	int (*IoChstat)(PspIoDrvFileArg *arg, byte *file, SceIoStat *stat, int bits);
	int (*IoRename)(PspIoDrvFileArg *arg, byte *oldname, byte *newname); 
	int (*IoChdir)(PspIoDrvFileArg *arg, byte *dir); 
	int (*IoMount)(PspIoDrvFileArg *arg); 
	int (*IoUmount)(PspIoDrvFileArg *arg); 
	int (*IoDevctl)(PspIoDrvFileArg *arg, byte *devname, uint cmd, void *indata, int inlen, void *outdata, int outlen); 
...
/**
  ...
  */
int sceIoReopen(byte *file, int flags, SceMode mode, SceUID fd);
 ...
 * returned.
 */
int sceIoGetThreadCwd(SceUID uid, byte *dir, int len);
 ...
 */
int sceIoChangeThreadCwd(SceUID uid, byte *dir);
