...
/**
  ...
  */
int pspDebugScreenPrintData(byte *buff, int size);
 ...
 * @return The number of characters written
 */
int pspDebugScreenPuts(byte *str);
...
alias _PspDebugRegBlock PspDebugRegBlock;
...
typedef int (*PspDebugKprintfHandler)(byte *format, u32 *args);
...
alias _PspDebugStackTrace PspDebugStackTrace;
...
{
	// One will simply have to use D's volatile{ } block around using this
	u32 enable;
	u32 systemck;
	u32 cpuck;
	u32 internal;
...
alias _PspDebugProfilerRegs PspDebugProfilerRegs;
...
typedef int (*PspDebugPrintHandler)(byte *data, int len);

/** Type for the debug input handler */
typedef int (*PspDebugInputHandler)(byte *data, int len);
 ...
 */
int pspDebugSioPutData(byte *data, int len);
 ...
 * @return Number of characters written.
 */
int pspDebugSioPutText(byte *data, int len);
