...
typedef int (*PspSysEventHandlerFunc)(int ev_id, byte* ev_name, void* param, int* result);
 
struct PspSysEventHandler{
  int size;
  byte* name;
  int type_mask;
  int (*handler)(int ev_id, byte* ev_name, void* param, int* result);
  int r28;
  int busy;
 ...
 */
int sceKernelSysEventDispatch(int ev_type_mask, int ev_id, byte* ev_name, void* param, int* result, int break_nonzero, PspSysEventHandler* break_handler);
 ...
 */
PspSysEventHandler* sceKernelReferSysEventHandler();
