...
module pspsdk.pspiofilemgr_stat;
...
template FIO_S_ISLNK(uint m) { 
	const bool FIO_S_ISLNK = (((m) & FIO_S_IFMT) == FIO_S_IFLNK);
}
template FIO_S_ISREG(uint m) {
	const bool FIO_S_ISREG = (((m) & FIO_S_IFMT) == FIO_S_IFREG);
}
template FIO_S_ISDIR(uint m) {
	const bool FIO_S_ISDIR = (((m) & FIO_S_IFMT) == FIO_S_IFDIR);
}
...
// File mode checking macros
template FIO_SO_ISLNK(uint m) {
	const bool FIO_SO_ISLNK=(((m) & FIO_SO_IFMT) == FIO_SO_IFLNK);
}
template FIO_SO_ISREG(uint m) {
	const bool FIO_SO_ISREG=(((m) & FIO_SO_IFMT) == FIO_SO_IFREG);
}
template FIO_SO_ISDIR(uint m) { 
	const bool FIO_SO_ISDIR = (((m) & FIO_SO_IFMT) == FIO_SO_IFDIR);
}
