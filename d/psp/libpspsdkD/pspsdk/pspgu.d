...
template GU_TEXTURE_SHIFT(int n) { const int GU_TEXTURE_SHIFT = n << 0; }
...
template GU_COLOR_SHIFT(int n) { const int GU_COLOR_SHIFT=n<<2; }
...
template GU_NORMAL_SHIFT(int n) { const int GU_NORMAL_SHIFT=n<<5; }
...
template GU_VERTEX_SHIFT(int n) { const int GU_VERTEX_SHIFT=n<<7; }
...
template GU_WEIGHT_SHIFT(int n) { const int GU_WEIGHT_SHIFT=n<<9; }
...
template GU_INDEX_SHIFT(int n) { const int GU_INDEX_SHIFT=n<<11; }
...
template GU_WEIGHTS(int n) { const int GU_WEIGHTS=((((n)-1)&7)<<14); }
const int GU_WEIGHTS_BITS =		GU_WEIGHTS!(8);
template GU_VERTICES(int n) { const int GU_VERTICES=((((n)-1)&7)<<18); }
const int GU_VERTICES_BITS =	GU_VERTICES!(8);

template GU_TRANSFORM_SHIFT(int n) { const int GU_TRANSFORM_SHIFT=((n)<<23); }
...
template GU_ABGR(ubyte a, ubyte b, ubyte g,ubyte r) {
	const uint GU_ABGR = (((a) << 24)|((b) << 16)|((g) << 8)|(r));
}
template GU_ARGB(ubyte a, ubyte r, ubyte g, ubyte b) {
//	const uint GU_ARGB = (((a) << 24)|((r) << 16)|((g) << 8)|(b));
	const uint GU_ARGB = GU_ABGR!(a,b,g,r);
// #define GU_ARGB(a,r,g,b)	GU_ABGR((a),(b),(g),(r))
}
template GU_RGBA(ubyte r,ubyte g, ubyte b, ubyte a) {
	// const uint GU_RGBA = (((r) << 24)|((g) << 16)|((b) << 8)|(a));
	const uint GU_RGBA = GU_ARGB!(a,r,g,b);
}
// #define GU_RGBA(r,g,b,a)	GU_ARGB((a),(r),(g),(b))

/* Color Macro, maps floating point channels (0..1) into one 32-bit value */
template GU_COLOR(float r, float g, float b, float a) {
	const uint GU_COLOR = GU_RGBA!(cast(u32)(r * 255.0f),
		cast(u32)(g * 255.0f), cast(u32)(b * 255.0f), cast(u32)(a * 255.0f)
	);
}
...
/**
  ...
  *
  * @param func - Depth test function to use
**/
...
**/
void sceGuStencilFunc(int func, int ref_, int mask);
